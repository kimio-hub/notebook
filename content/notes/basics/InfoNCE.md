...
CLIP 用的是**对称 InfoNCE**：一个 batch 里 $N$ 对图文，图→文方向和文→图方向各算一次（batch 内其余样本互为负样本），再取平均。训练出来的联合嵌入空间，就是后续一大批检索工作的地基——[[CoLLM]]、[[OSrCIR]] 这类 CIR 方法能"零样本"工作，靠的正是这个对比空间。

## 相关笔记

- [[Transformer]]：编码器骨架
- [[CoLLM]] / [[OSrCIR]]：建立在 CLIP 对比空间上的组合图像检索
